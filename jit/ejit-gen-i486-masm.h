...
unsigned char *ejit_allocate_local_register(unsigned char *inst, jit_function_t func, ejit_vreg_t vreg, ejit_vreg_t vreg1, ejit_vreg_t vreg2, unsigned char bUsage, unsigned int fRegCond, int typeKind, unsigned int *regFound);
...
static unsigned int count_free_gpregs_index(void *handler)
{
        ...
        {
            unsigned int reg_is_free = ejit_gp_reg_index_is_free(handler, index);
...
static unsigned int count_free_xmmregs_index(void *handler)
{
        ...
        {
            unsigned int reg_is_free = ejit_xmm_reg_index_is_free(handler, index);
...
{
    inst = ejit_allocate_local_register(inst, func, 0, 0, 0, LOCAL_ALLOCATE_FOR_TEMP, 0, JIT_TYPE_PTR, &gpreg1);
    return inst;
}
...
{
    unsigned int cond = ejit_gp_regs_map[gp_reg_map[cond1]].hash_code;
    inst = ejit_allocate_local_register(inst, func, 0, 0, 0, LOCAL_ALLOCATE_FOR_TEMP, cond, JIT_TYPE_PTR, &gpreg1);
    return inst;
}
...
{
    unsigned int cond = ejit_gp_regs_map[gp_reg_map[cond1]].hash_code;
    cond |= ejit_gp_regs_map[gp_reg_map[cond2]].hash_code;
    inst = ejit_allocate_local_register(inst, func, 0, 0, 0, LOCAL_ALLOCATE_FOR_TEMP, cond, JIT_TYPE_PTR, &gpreg1);
    return inst;
}
...
static unsigned char* __find_one_gp_reg_cond3(jit_function_t func, local_regs_allocator_t lrs, unsigned char *inst, unsigned char cond1, unsigned char cond2, unsigned char cond3)
{
    unsigned int cond = ejit_gp_regs_map[gp_reg_map[cond1]].hash_code;
    cond |= ejit_gp_regs_map[gp_reg_map[cond2]].hash_code;
    cond |= ejit_gp_regs_map[gp_reg_map[cond3]].hash_code;
    inst = ejit_allocate_local_register(inst, func, 0, 0, 0, LOCAL_ALLOCATE_FOR_TEMP, cond, JIT_TYPE_PTR, &gpreg1);
    return inst;
}
...
static unsigned char* __find_one_gp_reg_cond4(jit_function_t func, local_regs_allocator_t lrs, unsigned char *inst, unsigned char cond1, unsigned char cond2, unsigned char cond3, unsigned char cond4)
{
    ...
    cond |= ejit_gp_regs_map[gp_reg_map[cond2]].hash_code;
    cond |= ejit_gp_regs_map[gp_reg_map[cond3]].hash_code;
    cond |= ejit_gp_regs_map[gp_reg_map[cond4]].hash_code;
    inst = ejit_allocate_local_register(inst, func, 0, 0, 0, LOCAL_ALLOCATE_FOR_TEMP, cond, JIT_TYPE_PTR, &gpreg1);
    return inst;
}
...
static unsigned char* __find_one_gp_reg_cond6(jit_function_t func, local_regs_allocator_t lrs, unsigned char *inst, unsigned char cond1, unsigned char cond2, unsigned char cond3, unsigned char cond4, unsigned char cond5, unsigned char cond6)
{
    ...
    cond |= ejit_gp_regs_map[gp_reg_map[cond3]].hash_code;
    cond |= ejit_gp_regs_map[gp_reg_map[cond4]].hash_code;
    cond |= ejit_gp_regs_map[gp_reg_map[cond5]].hash_code;
    cond |= ejit_gp_regs_map[gp_reg_map[cond6]].hash_code;
    inst = ejit_allocate_local_register(inst, func, 0, 0, 0, LOCAL_ALLOCATE_FOR_TEMP, cond, JIT_TYPE_PTR, &gpreg1);
    return inst;
}
...
{
    inst = ejit_allocate_local_register(inst, func, 0, 0, 0, LOCAL_ALLOCATE_FOR_TEMP, 0, JIT_TYPE_PTR, &gpreg1);
    unsigned int cond = ejit_gp_regs_map[gp_reg_map[gpreg1]].hash_code;
    inst = ejit_allocate_local_register(inst, func, 0, 0, 0, LOCAL_ALLOCATE_FOR_TEMP, cond, JIT_TYPE_PTR, &gpreg2);
    return inst;
}
...
static unsigned char* __find_two_gp_regs_cond1(jit_function_t func, local_regs_allocator_t lrs, unsigned char *inst, unsigned char cond1)
{
    unsigned int cond = ejit_gp_regs_map[gp_reg_map[cond1]].hash_code;
    inst = ejit_allocate_local_register(inst, func, 0, 0, 0, LOCAL_ALLOCATE_FOR_TEMP, cond, JIT_TYPE_PTR, &gpreg1);
    cond |= ejit_gp_regs_map[gp_reg_map[gpreg1]].hash_code;
    inst = ejit_allocate_local_register(inst, func, 0, 0, 0, LOCAL_ALLOCATE_FOR_TEMP, cond, JIT_TYPE_PTR, &gpreg2);
    return inst;
}
...
{
    unsigned int regCond1 = ejit_gp_regs_map[gp_reg_map[cond1]].hash_code;
    unsigned int regCond2 = regCond1 | ejit_gp_regs_map[gp_reg_map[cond2]].hash_code;
    regCond2 |= ejit_gp_regs_map[gp_reg_map[cond3]].hash_code;

    inst = ejit_allocate_local_register(inst, func, 0, 0, 0, LOCAL_ALLOCATE_FOR_TEMP, regCond2, JIT_TYPE_PTR, &gpreg1);
    regCond1 |= ejit_gp_regs_map[gp_reg_map[gpreg1]].hash_code;
    inst = ejit_allocate_local_register(inst, func, 0, 0, 0, LOCAL_ALLOCATE_FOR_TEMP, regCond1, JIT_TYPE_PTR, &gpreg2);
    return inst;
}


#define release_two_gp_regs(inst) inst = ejit_restore_local_registers(inst, func, ejit_gp_regs_map[gp_reg_map[gpreg1]].hash_code | ejit_gp_regs_map[gp_reg_map[gpreg2]].hash_code);

#define release_one_gp_reg(inst) inst = ejit_restore_local_registers(inst, func, ejit_gp_regs_map[gp_reg_map[gpreg1]].hash_code);

static unsigned char *__find_one_xmm_reg(jit_function_t func, local_regs_allocator_t lrs, unsigned char *inst)
{
    inst = ejit_allocate_local_register(inst, func, 0, 0, 0, LOCAL_ALLOCATE_FOR_TEMP, 0, JIT_TYPE_FLOAT64, &xmmreg1);
    return inst;
}
...
{
    unsigned int cond = ejit_xmm_regs_map[cond1].hash_code;
    inst = ejit_allocate_local_register(inst, func, 0, 0, 0, LOCAL_ALLOCATE_FOR_TEMP, cond, JIT_TYPE_FLOAT64, &xmmreg1);
    return inst;
}

#define release_one_xmm_reg(inst) inst = ejit_restore_local_registers(inst, func, ejit_xmm_regs_map[xmmreg1].hash_code);

#define set_gpreg_as_scratched(gpreg) \
    func->ejit->scratch_regs |= (0x1 << gp_reg_map[gpreg]); \
...
static unsigned char *output_branch_from_label
    (jit_function_t func, unsigned char *inst, int opcode, jit_label_t label)
{
    jit_block_t block;
    int offset;
...
unsigned char *ejit_memory_copy_with_reg
...
unsigned char *ejit_memory_copy
    (unsigned char *inst, int dreg, jit_nint doffset,
     int sreg, jit_nint soffset, jit_nuint size, int temp_reg)
{
    if(size <= 4 * sizeof(void *))
...
// Registers EAX, EDX and ECX are scratched if an external function is called
// to perform this operation.
unsigned char *ejit_memory_copy_to_mem
...
// Registers EAX, EDX and ECX are scratched if an external function is called
// to perform this operation.
unsigned char *ejit_memory_copy_from_mem
    (unsigned char *inst, int dreg, jit_nint doffset,
...
unsigned char * _masm_mov_membase_membase(jit_function_t func, local_regs_allocator_t lrs, unsigned char *inst, int dreg, jit_nint doffset, int sreg, jit_nint soffset, jit_nuint size)
{
    if(size > 0 && (dreg != sreg || doffset != soffset))
    {
        ...
        {
            inst = ejit_memory_copy(inst, dreg, doffset, sreg, soffset, size, (int)(gpreg1));
        }
...
unsigned char *masm_mov_membase_reg(unsigned char *inst, unsigned char basereg, unsigned int offset, unsigned char sreg, jit_type_t type)
{
    ...
    switch(typeKind)
    {
        ...
        {
            x86_mov_membase_reg(inst, basereg, offset, sreg, 4);
            x86_mov_membase_reg(inst, basereg, offset + 4, ejit_gp_regs_map[gp_reg_map[sreg] + 1].reg, 4);
        }
        break;
...
unsigned char *masm_mov_reg_membase(unsigned char *inst, unsigned char dreg, unsigned char basereg, unsigned int offset, jit_type_t type)
{
    ...
    switch(typeKind)
    {
        ...
        CASE_USE_LONG
        {
            ...
            {
                x86_mov_reg_membase(inst, dreg, basereg, offset, 4);
                x86_mov_reg_membase(inst, ejit_gp_regs_map[gp_reg_map[dreg] + 1].reg, basereg, offset + 4, 4);
            }
            else
            {
                x86_mov_reg_membase(inst, ejit_gp_regs_map[gp_reg_map[dreg] + 1].reg, basereg, offset + 4, 4);
        ...
        CASE_USE_LONG
        {
            if(dreg != sreg)
            {
                if(gp_reg_map[dreg] < gp_reg_map[sreg])
                {
                    ...
                    x86_mov_reg_reg(inst, ejit_gp_regs_map[gp_reg_map[dreg] + 1].reg, ejit_gp_regs_map[gp_reg_map[sreg] + 1].reg, 4);
                }
                else
                {
                    x86_mov_reg_reg(inst, ejit_gp_regs_map[gp_reg_map[dreg] + 1].reg, ejit_gp_regs_map[gp_reg_map[sreg] + 1].reg, 4);
...
unsigned char* masm_mov_reg_imm(unsigned char *inst, unsigned char dreg, jit_nint address, jit_type_t type)
{
    ...
    switch(typeKind)
    {
        CASE_USE_LONG
        {
            ...
            x86_mov_reg_imm(inst, ejit_gp_regs_map[gp_reg_map[dreg] + 1].reg, ((jit_nint *)(address))[1]);
        }
        break;
        case JIT_TYPE_FLOAT32:
        {
...
unsigned char *masm_alu_reg_reg(unsigned char *inst, unsigned int opc1, unsigned int opc2, unsigned int dreg, unsigned int sreg)
{
    ...
    if(opc2) x86_alu_reg_reg(inst, opc2, ejit_gp_regs_map[gp_reg_map[dreg] + 1].reg, ejit_gp_regs_map[gp_reg_map[sreg] + 1].reg);
...
unsigned char *masm_alu_reg_imm(unsigned char *inst, unsigned int opc1, unsigned int opc2, unsigned int dreg, jit_nint address)
{
    if(opc2)
    {
        ...
        x86_alu_reg_imm(inst, opc2, ejit_gp_regs_map[gp_reg_map[dreg] + 1].reg, ((jit_nint *)(address))[1]);
    }
    else
    {
...
unsigned char *masm_alu_reg_membase(unsigned char *inst, unsigned int opc1, unsigned int opc2, unsigned int dreg, unsigned int basereg, unsigned int offset)
{
    ...
    if(opc2) x86_alu_reg_membase(inst, opc2, ejit_gp_regs_map[gp_reg_map[dreg] + 1].reg, basereg, offset + 4);
...
unsigned char *masm_alu_membase_reg(unsigned char *inst, unsigned int opc1, unsigned int opc2, unsigned int basereg, unsigned int offset, unsigned int sreg)
{
    ...
    if(opc2) x86_alu_membase_reg(inst, opc2, basereg, offset + 4, ejit_gp_regs_map[gp_reg_map[sreg] + 1].reg);
...
unsigned char* masm_mov_st0_value(unsigned char *inst, jit_value_t value)
{
    ...
    else if(ejit_value_in_frame(value))
    {
        jit_type_t type = jit_value_get_type(value);
        type = jit_type_remove_tags(type);
        int typeKind = jit_type_get_kind(type);
...
unsigned char *masm_push_qword(unsigned char *inst, jit_value_t value)
{
    if(value->vreg && value->vreg->in_reg)
    {
        ...
        x86_mov_membase_reg(inst, X86_ESP, 4, ejit_gp_regs_map[gp_reg_map[value->vreg->reg->reg] + 1].reg, 4);
    }
    else if(value->vreg && value->vreg->in_frame)
    {
        x86_push_membase(inst, X86_EBP, value->vreg->frame->frame_offset + 4);
...
//            }
    ...
    else if(imm == -1)
    {
        ...
        inst = ejit_throw_builtin(inst, func, JIT_RESULT_ARITHMETIC);
        x86_patch(patch, inst);
        x86_mov_reg_reg(inst, dreg, sreg, 4);
        x86_neg_reg(inst, dreg);
    }
    ...
    {
        inst = ejit_throw_builtin(inst, func, JIT_RESULT_DIVISION_BY_ZERO);
    }
