...
{
    incoming_params_state_t state = func->ejit->incoming_params_state;
    if(func->ejit->incoming_params_state == 0)
    {
        state = jit_cnew(struct _incoming_params_state);
        func->ejit->incoming_params_state = state;
    }
    ...
    ejit_linked_list_t linked_list = func->ejit->vregs_list;
    while(linked_list)
    {
        ejit_vreg_t vreg = (ejit_vreg_t)linked_list->item;
        ejit_free_frame(func, vreg->frame);
        linked_list = linked_list->next;
    }

    func->ejit->scratch_regs = 0;
    func->ejit->scratch_frame = 0;

    int index;
    for(index = 0; index < EJIT_N_GP_REGISTERS; index++)
    {
        ejit_gp_regs_map[index].vreg = 0;
        ejit_gp_regs_map[index].local_vreg = 0;
        ejit_free_frame(func, ejit_gp_regs_map[index].temp_frame);
        ejit_gp_regs_map[index].temp_frame = 0;
    }

    for(index = 0; index < EJIT_N_XMM_REGISTERS; index++)
    {
        ejit_xmm_regs_map[index].vreg = 0;
        ejit_xmm_regs_map[index].local_vreg = 0;
        ejit_free_frame(func, ejit_xmm_regs_map[index].temp_frame);
        ejit_xmm_regs_map[index].temp_frame = 0;
    }

    func->ejit->relative_sp_offset = 0;
}
...
void ejit_reinit(jit_function_t func)
{
    ...
    {
        ejit_gp_regs_map[index].vreg = 0;
        ejit_vreg_t local_vreg = ejit_gp_regs_map[index].local_vreg;
        if(local_vreg)
        {
    ...
    for(index = 0; index < EJIT_N_XMM_REGISTERS; index++)
    {
        ejit_xmm_regs_map[index].vreg = 0;
        ejit_xmm_regs_map[index].local_vreg = 0;
        ejit_vreg_t local_vreg = ejit_xmm_regs_map[index].local_vreg;
        if(local_vreg)
        {
            local_vreg->in_frame = 1;
            local_vreg->in_reg = 0;
...
unsigned char ejit_gp_reg_is_free(jit_function_t func, int index) 
{
    return (func->ejit->regs_state & (0x1 << gp_reg_map[index])) == 0;
}

unsigned char ejit_xmm_reg_is_free(jit_function_t func, int index)
{
    return (func->ejit->regs_state & (64 << index)) == 0;
}

unsigned char ejit_gp_reg_index_is_free(void *handler, int index) 
{
    return (((jit_function_t)handler)->ejit->regs_state & (0x1 << index)) == 0;
}

unsigned char ejit_xmm_reg_index_is_free(void *handler, int index)
{
    return (((jit_function_t)handler)->ejit->regs_state & (64 << index)) == 0;
}

unsigned char ejit_register_is_free(jit_function_t func, int reg, jit_value_t value)
{
    ...
    switch(typeKind)
    {
        CASE_USE_WORD
        {
            return ejit_gp_reg_is_free(func, reg);
        }
        break;
        CASE_USE_FLOAT
        {
            return ejit_xmm_reg_is_free(func, reg);
        }
        break;
        CASE_USE_LONG
        {
            return (ejit_gp_reg_is_free(func, reg) && ejit_gp_reg_index_is_free(func, gp_reg_map[reg] + 1));
        }
        break;
    }
    return 0;
...
unsigned char ejit_regIndex_is_free(jit_function_t func, int regIndex, jit_value_t value)
{
    jit_type_t type = jit_value_get_type(value);
    type = jit_type_remove_tags(type);
    int typeKind = jit_type_get_kind(type);
    switch(typeKind)
    {
        CASE_USE_WORD
        {
            return ejit_gp_reg_index_is_free(func, regIndex);
        }
        break;
        CASE_USE_FLOAT
        {
            return ejit_xmm_reg_index_is_free(func, regIndex);
        }
        break;
        CASE_USE_LONG
        {
            return (ejit_gp_reg_index_is_free(func, regIndex) && ejit_gp_reg_index_is_free(func, regIndex + 1));
        }
        break;
    }
    return 0;
...
int ejit_regIndex2reg(jit_function_t func, int regIndex, jit_type_t type)
{
    ...
    switch(typeKind)
    {
        CASE_USE_WORD
        {
            return ejit_gp_regs_map[regIndex].reg;
        }
        break;
        CASE_USE_FLOAT
        {
            return X86_REG_XMM0 + ejit_xmm_regs_map[regIndex].reg;
        }
        break;
    }
    return -1;
...
unsigned char ejit_regIndex_CanBeUsedOnInput(jit_function_t func, unsigned int regIndex, jit_value_t value)
{
    jit_type_t signature = jit_function_get_signature(func);
    unsigned int num = ejit_type_num_params(signature);
    int count;
    int index = 0;
    jit_abi_t abi = jit_function_get_abi(func);
    if(!ejit_regIndex_is_free(func, regIndex, value)) return 0;
    else if((regIndex > 2 || abi != jit_abi_internal) && !ejit_vreg_is_in_register_hole(func, value->vreg, regIndex)) return 1;
    else if(regIndex > 2 || abi != jit_abi_internal) return 0;
    for(count = 0; (index <= regIndex) && (count < num); count++)
    {
        jit_type_t type = ejit_type_get_param(signature, count);
        type = jit_type_remove_tags(type);
        int typeKind = jit_type_get_kind(type);
        switch(typeKind)
        {
            CASE_USE_WORD
            {
                if(index == regIndex)
                {
                    jit_value_t param = ejit_value_get_param(func, count);
    ...
    if(!ejit_vreg_is_in_register_hole(func, value->vreg, regIndex)) return 1;
    return 0;
}
                   ...
                   jit_block_t block, jit_insn_t insn)
{
	...
	break;
        ...
        case JIT_OP_INCOMING_FRAME_POSN:
        {
            ...
            {
                ejit_frame_t frame = jit_memory_pool_alloc(&(func->builder->ejit_frame_pool), struct _ejit_frame);            
                frame->hash_code = -1;
                frame->frame_offset = (int)frame_offset;
                frame->length = ejit_type_get_size(type) / 4;
                value->vreg->frame = frame;
            }
            else if(value->vreg->in_reg)
            {
                func->ejit->regs_state = func->ejit->regs_state | value->vreg->reg->hash_code;
                func->ejit->scratch_regs = func->ejit->scratch_regs | value->vreg->reg->hash_code;
        ...
        case JIT_OP_INCOMING_REG:
        {
            ...
            int size = ejit_type_get_size(type);
            ...
            {
                ejit_frame_t frame = jit_memory_pool_alloc(&(func->builder->ejit_frame_pool), struct _ejit_frame);
                ejit_allocate_large_frame(func, frame, size);
                value->vreg->frame = frame;
                unsigned char *inst = (unsigned char *)(gen->posn.ptr);
        ...
        default:
        {
            ...
            if(insn->cpoint)
            {
                ejit_free_frames(func, insn->cpoint->vregs_die);
                if((insn->flags & JIT_INSN_DEST_IS_LABEL) == 0)
                {
                    ejit_free_registers(func, insn->cpoint->vregs_die);
                }

                inst = ejit_allocate_registers_and_frames(inst, func, insn->cpoint->vregs_born->item1);
                inst = ejit_allocate_registers_and_frames(inst, func, insn->cpoint->vregs_born->item2);
                inst = ejit_allocate_registers_and_frames(inst, func, insn->cpoint->vregs_born->item3);
                inst = ejit_allocate_registers_and_frames(inst, func, insn->cpoint->vregs_born->item4);
            ...
            switch(insn->opcode)
            {
                ...
                {
                    inst = ejit_allocate_local_register(inst, func, value2->vreg, value1->vreg, value2->vreg, LOCAL_ALLOCATE_FOR_INPUT,  0, 0, 0);
                    inst = ejit_allocate_local_register(inst, func, value1->vreg, value1->vreg, value2->vreg, LOCAL_ALLOCATE_FOR_INPUT,  0, 0, 0);
                    inst = ejit_allocate_local_register(inst, func, dest->vreg, value1->vreg, value2->vreg,   LOCAL_ALLOCATE_FOR_OUTPUT, 0, 0, 0);
                }
                break;
                case JIT_OP_LOAD_RELATIVE_STRUCT:
                {
                    ...
                    // restore EAX, EDX
                    if(value1->vreg && value1->vreg->in_frame) inst = ejit_restore_local_registers(inst, func, 0x3);
                    else if(!value1->vreg || (value1->vreg && value1->vreg->in_reg && value1->vreg->reg->hash_code != 0x1)) inst = ejit_restore_local_registers(inst, func, 0x1);
                    else inst = ejit_restore_local_registers(inst, func, 0x2);
                    inst = ejit_restore_local_registers(inst, func, 0x7);
                    ...
                    inst = ejit_allocate_local_register(inst, func, value1->vreg, 0, 0, LOCAL_ALLOCATE_FOR_INPUT, 0, 0, 0);
                }
                break;
                ...
                case JIT_OP_MEMCPY:
                {
                    inst = ejit_allocate_local_register(inst, func, dest->vreg, value1->vreg, value2->vreg, LOCAL_ALLOCATE_FOR_INPUT, 0, 0, 0);
                    inst = ejit_allocate_local_register(inst, func, value1->vreg, value2->vreg, dest->vreg, LOCAL_ALLOCATE_FOR_INPUT, 0, 0, 0);
                    inst = ejit_allocate_local_register(inst, func, value2->vreg, value1->vreg, dest->vreg, LOCAL_ALLOCATE_FOR_INPUT, 0, 0, 0);
                }
                break;
                ...
                case JIT_OP_STORE_RELATIVE_STRUCT:
                {
                    ...
                    // restore EAX, EDX
                    if(dest->vreg && dest->vreg->in_frame) inst = ejit_restore_local_registers(inst, func, 0x3);
                    else if(!dest->vreg || (dest->vreg && dest->vreg->in_reg && dest->vreg->reg->hash_code != 0x1)) inst = ejit_restore_local_registers(inst, func, 0x1);
                    else inst = ejit_restore_local_registers(inst, func, 0x2);
                ...
                case JIT_OP_COPY_STRUCT:
                {
                    ...
                    {
                        // restore edx and ecx
                        inst = ejit_restore_local_registers(inst, func, 0x6);
                    }
                }
                break;
                ...
                {
                    inst = ejit_restore_local_registers(inst, func, 0x3fffc7);
                }
                break;
                case JIT_OP_IDIV:
                case JIT_OP_IDIV_UN:
                ...
                {
                    // restore EAX, EDX
                    inst = ejit_restore_local_registers(inst, func, 0x3);
                }
                break;
                case JIT_OP_LDIV:
                case JIT_OP_LDIV_UN:
                {
                    ...
                    inst = ejit_restore_local_registers(inst, func, 0x3f);
                }
                break;
                case JIT_OP_LMUL:
                {
                    // Restore EAX, EDX, ECX and EBX
                    inst = ejit_restore_local_registers(inst, func, 0x15);
                }
                break;
                case JIT_OP_LSHR:
                case JIT_OP_LSHR_UN:
                ...
                {
                    inst = ejit_restore_local_registers(inst, func, 0xffffffff);
                }
                break;
            ...
            if(value2)
            {
                ...
                if(value2->vreg)
                {
                    inst = ejit_restore_local_vreg(inst, func, value2->vreg);
            ...
            unsigned char machine_code1 = ejit_opcodes_map[insn->opcode].machine_code1;
            unsigned char machine_code2 = ejit_opcodes_map[insn->opcode].machine_code2;
            unsigned char machine_code3 = ejit_opcodes_map[insn->opcode].machine_code3;
            unsigned char machine_code4 = ejit_opcodes_map[insn->opcode].machine_code4;
	    ...
	    if(insn->opcode == JIT_OP_OUTGOING_REG)
	    {
            ...
            {
                inst = ejit_restore_local_frame(inst, func, dest->vreg);
            }
...
void *gen_prolog(jit_gencode_t gen, jit_function_t func, void *buf)
{
    ...
    int stack_offset = func->ejit->scratch_frame;

    for(index = 3; index < EJIT_N_GP_REGISTERS; index++)
    {
        if(func->ejit->scratch_regs & ejit_gp_regs_map[index].hash_code)
        {
            stack_offset+=4;
            x86_mov_membase_reg(inst, X86_EBP, -stack_offset, ejit_gp_regs_map[index].reg, 4);
        }
    }
...
unsigned char *restore_callee_saved_registers(unsigned char *inst, jit_function_t func)
{
    ...
    for(index = 3; index < EJIT_N_GP_REGISTERS; index++)
    {
        ...
        {
            stack_offset+=4;
            x86_mov_reg_membase(inst, ejit_gp_regs_map[index].reg, X86_EBP, -stack_offset, 4);
        }
    }
...
void gen_epilog(jit_gencode_t gen, jit_function_t func)
{
    ...
    if(func->ejit->scratch_frame > 0 || stack_offset > 0 || gen->stack_changed)
    {
        x86_mov_reg_reg(inst, X86_ESP, X86_EBP, sizeof(void *));
    }
    x86_pop_reg(inst, X86_EBP);
    
    int pop_bytes = ejit_stack_depth_used(func);
...
void gen_end_block(jit_gencode_t gen, jit_block_t block)
{
    ...
    inst = ejit_restore_local_registers(inst, block->func, 0xffffffff);
    ...
    if(vreg)
    {
        ...
        if(vreg->in_reg)
        {
                if(vreg->reg->vreg == vreg)
                {
                    ...
                    {
                        ejit_reg_t reg_pair = ejit_object_register_pair(vreg->reg);
                        func->ejit->regs_state = func->ejit->regs_state & (~(reg_pair->hash_code));
                        reg_pair->vreg = 0;
                    }
                }
                else if(vreg->reg->temp_frame == 0)
                {
                    ...
                    if(typeKind == JIT_TYPE_LONG || typeKind == JIT_TYPE_ULONG)
                    {
                        func->ejit->regs_state = func->ejit->regs_state & (~(ejit_object_register_pair(vreg->reg)->hash_code));
                    }
                }
        }
    }
...
unsigned char *ejit_restore_local_registers(unsigned char *inst, jit_function_t func, unsigned int regMask)
{
    ...
    {
        ejit_vreg_t local_vreg = ejit_gp_regs_map[index].local_vreg;
        if(local_vreg && (ejit_gp_regs_map[index].hash_code & regMask))
        {
            local_vreg->in_reg = 0;
            local_vreg->in_frame = 1;
            ejit_gp_regs_map[index].local_vreg = 0;
            if(ejit_gp_regs_map[index].temp_frame == 0)
            {
                func->ejit->regs_state = func->ejit->regs_state & (~(ejit_gp_regs_map[index].hash_code));
            }
            else
            {
                x86_mov_reg_membase(inst, local_vreg->reg->reg, X86_EBP, ejit_gp_regs_map[index].temp_frame->frame_offset, 4);
                ejit_free_frame(func, ejit_gp_regs_map[index].temp_frame);
            }
            local_vreg->reg = 0;
            ejit_gp_regs_map[index].temp_frame = 0;
    ...
    {
        ejit_vreg_t local_vreg = ejit_xmm_regs_map[index].local_vreg;
        if(local_vreg && (ejit_xmm_regs_map[index].hash_code & regMask))
        {
            local_vreg->in_reg = 0;
            local_vreg->in_frame = 1;
            ejit_xmm_regs_map[index].local_vreg = 0;
            if(ejit_xmm_regs_map[index].temp_frame == 0)
            {
            ...
            else
            {
                ...
                type = jit_type_remove_tags(type);
                inst = masm_mov_reg_membase(inst, local_vreg->reg->reg, X86_EBP, ejit_xmm_regs_map[index].temp_frame->frame_offset, type);
                ejit_free_frame(func, ejit_xmm_regs_map[index].temp_frame);
            }
            local_vreg->reg = 0;
            ejit_xmm_regs_map[index].temp_frame = 0;
        ...
        while((insn = jit_insn_iter_next(&iter)) != 0)
        {
            switch(insn->opcode)
            {
                ...
                switch(typeKind)
                {
                    CASE_USE_WORD
                    {
                        ...
                        if(value && value->vreg && value->vreg->min_range != value->vreg->max_range)
                        {
                            ...
                            {
                                if(!ejit_vreg_is_in_register_hole(func, value->vreg, regIndex) && ejit_regIndex_is_free(func, regIndex, value))
                                {
                                    indexFound = regIndex;
                                }
                                else
                                {
                                    ...
                                    for(index = 0; index < EJIT_N_GP_REGISTERS; index++)
                                    {
                                        if(!ejit_vreg_is_in_register_hole(func, value->vreg, index) && ejit_regIndex_is_free(func, index, value))
                                        {
                                            indexFound = index;
                                            break;
                                        }
                                ...
                                if(indexFound != -1)
                                {
                                    func->ejit->regs_state = func->ejit->regs_state | ejit_gp_regs_map[indexFound].hash_code;
                                    func->ejit->scratch_regs = func->ejit->scratch_regs | ejit_gp_regs_map[indexFound].hash_code;
                    ...
                    case JIT_TYPE_FLOAT64:
                    {
                        ...
                        if(value->vreg && value->vreg->min_range != value->vreg->max_range)
                        {
                            ...
                            if(!func->has_try && !jit_value_is_addressable(value))
                            {
                                ...
                                if(indexFound != -1)
                                {
                                    func->ejit->regs_state = func->ejit->regs_state | ejit_xmm_regs_map[indexFound].hash_code;
                ...
                if(insn->cpoint)
                {
                    ...
                    {
                        ejit_free_registers(func, insn->cpoint->vregs_die);
                        ejit_preallocate_registers_and_frames(func, insn->cpoint->vregs_born);
                    }
                    else
                    {
                        ejit_preallocate_registers_and_frames(func, insn->cpoint->vregs_born);
...
{
    int max = (EJIT_N_GP_REGISTERS > EJIT_N_XMM_REGISTERS ? EJIT_N_GP_REGISTERS: EJIT_N_XMM_REGISTERS);
    char reg_weight[max];
    memset(reg_weight, 0, sizeof(char) * max);
    ejit_vreg_t vregs[max];
    memset(vregs, 0, sizeof(ejit_vreg_t) * max);

    // Group of virtual registers that use general purpose registers.
    ejit_linked_list_t dword_list = list->item1;

    // Group of virtual registers that use 64-bit values.
    ejit_linked_list_t qword_list = list->item2;

    // Group of virtual registers that use XMM registers.
    ejit_linked_list_t float_list = list->item3;

    // Group of virtual registers that use frame and local memory.
    ejit_linked_list_t large_frames_list = list->item4;

    unsigned int need_n_gp = ejit_count_items(func, dword_list);
    ...
    for(index = 0; func && func->ejit && list
                && (index < EJIT_N_GP_REGISTERS) && dword_list && (need_n_gp > 0); index++)
    {
        if(ejit_gp_reg_index_is_free(func, index))
        {
            int use_reg = 0;
            ejit_vreg_t vreg;
            if(func && list && func->ejit)
            {
                ...
                while(temp)
                {
                    vreg = (ejit_vreg_t)(temp->item);
                    weight = ejit_vreg_weight(func, vreg);

                    if(!ejit_vreg_is_in_register_hole(func, vreg, index)
                ...
                if(use_reg == 1)
                {
                     ...
                     vregs[index]->reg = (ejit_reg_t)(&(ejit_gp_regs_map[index]));
                ...
                && (index < EJIT_N_GP_REGISTERS) && dword_list && (need_n_gp > 0); index++)
    {
        int found_index = -1;
        unsigned int count;
        for(count = 0; count < EJIT_N_GP_REGISTERS; count++)
        {
            // All free register were used, or cannot be used.
            // The register is not free, and was not just allocated.
            if(reg_weight[count] == 0 && !ejit_gp_reg_index_is_free(func, count))
            {
                if(found_index == -1
                    || ((found_index != -1) && ejit_vreg_weight(func, ejit_gp_regs_map[count].vreg)
                        > ejit_vreg_weight(func, ejit_gp_regs_map[found_index].vreg)))
                {
                    found_index = count;
                }
            }
        ...
        if(func && list && func->ejit)
        {
            ...
            while(temp)
            {
                ...
                if(ejit_vreg_weight(func, ejit_gp_regs_map[found_index].vreg) > weight
                    && !ejit_vreg_is_in_register_hole(func, vreg, found_index)
            ...
            if(use_reg == 1)
            {
                 ...
                 vregs[found_index]->reg = (ejit_reg_t)(&(ejit_gp_regs_map[found_index]));

                 ejit_gp_regs_map[found_index].vreg->in_reg = 0;
                 ejit_gp_regs_map[found_index].vreg->in_frame = 1;
    ...
    for(index = 0; func && func->ejit && list
                && (index < EJIT_N_GP_REGISTERS - 1) && qword_list && (need_n_long > 0); index++)
    {
        int index_pair = ejit_index_register_pair(index);
        if(ejit_gp_reg_index_is_free(func, index) && ejit_gp_reg_index_is_free(func, index_pair))
        {
            int use_reg = 0;
            ejit_vreg_t vreg;
            if(func && list && func->ejit)
            {
                ...
                if(use_reg == 1)
                {
                     ...
                     vregs[index]->in_frame = 0;
                     vregs[index]->reg = (ejit_reg_t)(&(ejit_gp_regs_map[index]));
                     ejit_gp_regs_map[index].vreg = vregs[index];
                     ejit_gp_regs_map[index_pair].vreg = vregs[index];
                ...
                && (index < EJIT_N_GP_REGISTERS - 1) && qword_list && (need_n_long > 0); index++)
    {
        ...
        unsigned int count;
        for(count = 0; count < EJIT_N_GP_REGISTERS - 1; count++)
        {
            // All free register were used, or cannot be used.
            // The register is not free, and was not just allocated.
            int count_pair = ejit_index_register_pair(count);
            if(reg_weight[count] == 0 && !ejit_gp_reg_index_is_free(func, count)
                && reg_weight[count_pair] == 0 && !ejit_gp_reg_index_is_free(func, count_pair))

            {
                if(found_index == -1 || ((found_index != -1)
                    && ejit_vreg_weight(func, ejit_gp_regs_map[count].vreg) > ejit_vreg_weight(func, ejit_gp_regs_map[found_index].vreg)
                    && ejit_vreg_weight(func, ejit_gp_regs_map[count_pair].vreg) > ejit_vreg_weight(func, ejit_gp_regs_map[count_pair].vreg)))
                {
                    found_index = count;
                    found_index_pair = ejit_index_register_pair(found_index);
                }
            }
        }
        if(found_index == -1) break;
        ...
        if(func && list && func->ejit)
            ...
            if(use_reg == 1)
            {
                 ...
                 ejit_gp_regs_map[found_index].vreg->in_reg = 0;
                 ejit_gp_regs_map[found_index].vreg->in_frame = 1;
                 ejit_gp_regs_map[found_index_pair].vreg->in_reg = 0;
                 ejit_gp_regs_map[found_index_pair].vreg->in_frame = 1;
    ...
    {
        if(ejit_xmm_reg_index_is_free(func, index))
        {
            int use_reg = 0;
            ejit_vreg_t vreg;
            if(func && list && func->ejit)
            {
                ...
                if(use_reg == 1)
                {
                     ...
                     vregs[index]->in_reg = 1;
                     vregs[index]->in_frame = 0;
                     vregs[index]->reg = (ejit_reg_t)(&(ejit_xmm_regs_map[index]));
                     vregs[index]->reg->vreg = vregs[index];
                ...
                && index < EJIT_N_XMM_REGISTERS && float_list && need_n_xmm > 0; index++)
    {
        int found_index = -1;
        unsigned int count;
        for(count = 0; count < EJIT_N_XMM_REGISTERS; count++)
        {
            // All free register were used, or cannot be used.
            // The register is not free, and was not just allocated.
            if(reg_weight[count] == 0 && !ejit_xmm_reg_index_is_free(func, count))
            {
                if(found_index == -1
                    || ((found_index != -1) && ejit_vreg_weight(func, ejit_xmm_regs_map[count].vreg)
                        > ejit_vreg_weight(func, ejit_xmm_regs_map[found_index].vreg)))
                {
                    found_index = count;
                }
            }
        ...
        if(func && list && func->ejit)
        {
            ...
            while(temp)
            {
                ...
                if(ejit_vreg_weight(func, ejit_xmm_regs_map[found_index].vreg) > weight
                    && !ejit_vreg_is_in_register_hole(func, vreg, found_index)
                    && (reg_weight[found_index] == 0 || weight < reg_weight[found_index])
                    && weight != 0 && vreg->in_reg==0 && !jit_value_is_addressable(vreg->value)
                    && (!func->has_try || jit_value_is_temporary(vreg->value)))
            ...
            if(use_reg == 1)
            {
                 ...
                 vregs[found_index]->in_reg = 1;
                 vregs[found_index]->in_frame = 0;
                 vregs[found_index]->reg = (ejit_reg_t)(&(ejit_xmm_regs_map[found_index]));

                 ejit_xmm_regs_map[found_index].vreg->in_reg = 0;
                 ejit_xmm_regs_map[found_index].vreg->in_frame = 1;
                 vregs[found_index]->reg->vreg = vregs[found_index];
...
unsigned char *ejit_allocate_registers_and_frames(unsigned char *inst, jit_function_t func, ejit_linked_list_t list)
{
    while(list)
    {
        ejit_vreg_t vreg = (ejit_vreg_t)list->item;
        if(vreg)
        {
            jit_type_t type = jit_value_get_type(vreg->value);
            type = jit_type_remove_tags(type);
            ...
            switch(typeKind)
            {
                CASE_USE_WORD
                {
                    if(vreg->in_frame)
                    {
                        ejit_frame_t frame = jit_memory_pool_alloc(&(func->builder->ejit_frame_pool), 
                                                struct _ejit_frame);
                        ejit_allocate_frame(func, frame);
                        vreg->frame = frame;
                    }
                    else if(vreg->in_reg)
                    {
                        ejit_gp_regs_map[vreg->reg->index].vreg = vreg;
                        func->ejit->regs_state = func->ejit->regs_state | vreg->reg->hash_code;
                ...
                CASE_USE_LONG
                {
                    if(vreg->in_frame)
                    {
                        ejit_frame_t frame = jit_memory_pool_alloc(&(func->builder->ejit_frame_pool), 
                                                struct _ejit_frame);
                        ejit_allocate_large_frame(func, frame, 8);
                        vreg->frame = frame;
                    }
                    else if(vreg->in_reg)
                    {
                        int reg_pair_index = ejit_index_register_pair(vreg->reg->index);
                        ejit_gp_regs_map[vreg->reg->index].vreg = vreg;
                        ejit_gp_regs_map[reg_pair_index].vreg = vreg;
                        func->ejit->regs_state = func->ejit->regs_state | vreg->reg->hash_code;
                        func->ejit->scratch_regs = func->ejit->scratch_regs | vreg->reg->hash_code;
                        func->ejit->regs_state = func->ejit->regs_state | (&ejit_gp_regs_map[reg_pair_index])->hash_code;
                        func->ejit->scratch_regs = func->ejit->scratch_regs | (&ejit_gp_regs_map[reg_pair_index])->hash_code;
                    }
                }
                break;
                case JIT_TYPE_FLOAT32:
                ...
                case JIT_TYPE_NFLOAT:
                {
                    if(sizeof(jit_nfloat) != sizeof(jit_float64))
                    {
                        if(vreg->in_frame)
                        {
                            ejit_frame_t frame = jit_memory_pool_alloc(&(func->builder->ejit_frame_pool), 
                                                    struct _ejit_frame);
                            ejit_allocate_large_frame(func, frame, 12);
                            vreg->frame = frame;
                        }
                    }
                    else
...
//                            }
                ...
                default:
                {
                    ejit_frame_t frame = jit_memory_pool_alloc(&(func->builder->ejit_frame_pool),
                                                struct _ejit_frame);
                    ejit_allocate_large_frame(func, frame, jit_type_get_size(jit_value_get_type(vreg->value)));
                    vreg->frame = frame;
                }
                break;
            }
...
void ejit_allocate_large_frame_cond6(jit_function_t func, ejit_frame_t frame, int size, ejit_frame_t frame1, ejit_frame_t frame2, ejit_frame_t frame3, ejit_frame_t frame4, ejit_frame_t frame5, ejit_frame_t frame6)
{
    ...
    ejit_list_t temp = func->ejit->frame_state;
    int segment, offset;
    ...
    while(temp && flag < size)
    {
        ...
        if(temp->next==0)
        {
            temp->next = jit_memory_pool_alloc(&(func->builder->ejit_list_pool),
                                    struct _ejit_list);
            temp->next->item1 = 0;
            temp->next->item2 = 0;
            temp->next->item3 = 0;
    ...
    frame->length = size;
    if((8 + (index * 4)) > func->ejit->scratch_frame) func->ejit->scratch_frame = 8 + (index * 4);
...
void ejit_allocate_large_frame(jit_function_t func, ejit_frame_t frame, int size)
{
    if(size <= 4)
    {
        ejit_allocate_frame(func, frame);
...
void ejit_allocate_frame(jit_function_t func, ejit_frame_t frame)
{
    ejit_list_t temp = func->ejit->frame_state;
    int segment, offset;
    unsigned int index = 0;
...
unsigned char *ejit_allocate_local_register(unsigned char *inst, jit_function_t func, ejit_vreg_t vreg, ejit_vreg_t vreg1, ejit_vreg_t vreg2, unsigned char bUsage, unsigned int fRegCond, int typeKind, unsigned int *foundReg)
{
    if((vreg && vreg->in_frame) || bUsage == LOCAL_ALLOCATE_FOR_TEMP)
    {
        ...
        if(vreg2 != 0 && vreg2->in_reg && vreg2->reg)
        {
            ...
            if(typeKind == JIT_TYPE_LONG || typeKind == JIT_TYPE_ULONG)
            {
                reg4 = ejit_object_register_pair(reg2);
            }
        }

        ejit_frame_t frame1 = 0, frame2 = 0, frame3 = 0, frame4 = 0, frame5 = 0, frame6 = 0;
        ...
        switch(typeKind)
        {
            CASE_USE_WORD
            {
                ...
                for(count = 0; count < EJIT_N_GP_REGISTERS; count++)
                {
                    if(ejit_gp_regs_map[count].vreg == 0 && ejit_gp_regs_map[count].local_vreg == 0 &&
                        &ejit_gp_regs_map[count] != reg1 && &ejit_gp_regs_map[count] != reg2 &&
                        &ejit_gp_regs_map[count] != reg3 && &ejit_gp_regs_map[count] != reg4 &&
                        !(ejit_gp_regs_map[count].hash_code & fRegCond))
                    {
                        ejit_gp_regs_map[count].local_vreg = vreg;
                        if(bUsage == LOCAL_ALLOCATE_FOR_INPUT) x86_mov_reg_membase(inst, ejit_gp_regs_map[count].reg, X86_EBP, vreg->frame->frame_offset, 4);
                        if(vreg)
                        {
                            vreg->in_frame = 0;
                            vreg->in_reg = 1;
                            vreg->reg = &ejit_gp_regs_map[count];
                        }

                        func->ejit->scratch_regs = func->ejit->scratch_regs | ejit_gp_regs_map[count].hash_code;
                        func->ejit->regs_state = func->ejit->regs_state | ejit_gp_regs_map[count].hash_code;
                        
                        bRegFound = 1;
                        if(foundReg) *foundReg = ejit_gp_regs_map[count].reg;
                ...
                if(!bRegFound) // if we did not find any free register unused
                {
                    for(count = 0; count < EJIT_N_GP_REGISTERS; count++)
                    {
                        if(ejit_gp_regs_map[count].vreg == 0 &&
                             &ejit_gp_regs_map[count] != reg1 && &ejit_gp_regs_map[count] != reg2 &&
                             &ejit_gp_regs_map[count] != reg3 && &ejit_gp_regs_map[count] != reg4 &&
                             !(ejit_gp_regs_map[count].hash_code & fRegCond))
                        {
                            // restore the old local register content to frame
                            ejit_vreg_t local_vreg = ejit_gp_regs_map[count].local_vreg;
                            if(local_vreg)
                            {
                                local_vreg->in_reg = 0;
                                local_vreg->in_frame = 1;
                ...
                if(!bRegFound) // if we did not find any free register at all try to find one already saved locally
                {
                    for(count = 0; count < EJIT_N_GP_REGISTERS; count++)
                    {
                        if(&ejit_gp_regs_map[count] != reg1 && &ejit_gp_regs_map[count] != reg2 &&
                           &ejit_gp_regs_map[count] != reg3 && &ejit_gp_regs_map[count] != reg4 &&
                           ejit_gp_regs_map[count].temp_frame &&
                           !(ejit_gp_regs_map[count].hash_code & fRegCond))
                        {
                            // load new content into local register
                            if(bUsage == LOCAL_ALLOCATE_FOR_INPUT) x86_mov_reg_membase(inst, ejit_gp_regs_map[count].reg, X86_EBP, vreg->frame->frame_offset, 4);
                ...
                if(!bRegFound) // if we did not find any free register at all try to find one not saved yet locally
                {
                    for(count = 0; count < EJIT_N_GP_REGISTERS; count++)
                    {
                        ...
                        {
                            // load new content into local register
                            ejit_frame_t frame = ejit_gp_regs_map[count].temp_frame;
                            if(frame == 0)
                            {
                                frame = jit_memory_pool_alloc(&(func->builder->ejit_frame_pool),
                                                    struct _ejit_frame);
                                ejit_allocate_large_frame_cond6(func, frame, ejit_type_get_size(jit_value_get_type(ejit_gp_regs_map[count].vreg->value)), frame1, frame2, frame3, frame4, frame5, frame6);
                                ejit_gp_regs_map[count].temp_frame = frame;
                            }
                            
                            jit_type_t type = jit_value_get_type(ejit_gp_regs_map[count].vreg->value);
                            type = jit_type_normalize(type);
                            int typeKind = jit_type_get_kind(type);
            ...
            CASE_USE_FLOAT
            {
                ...
                for(count = 0; count < EJIT_N_XMM_REGISTERS; count++)
                {
                    if(ejit_xmm_regs_map[count].vreg == 0 && ejit_xmm_regs_map[count].local_vreg == 0 &&
                         &ejit_xmm_regs_map[count] != reg1 && &ejit_xmm_regs_map[count] != reg2 &&
                       !(ejit_xmm_regs_map[count].hash_code & fRegCond))
                    {
                        ejit_xmm_regs_map[count].local_vreg = vreg;
                        if(typeKind == JIT_TYPE_FLOAT32)
                        {
                            if(bUsage == LOCAL_ALLOCATE_FOR_INPUT) sse_movss_xmreg_membase(inst, ejit_xmm_regs_map[count].reg, X86_EBP, vreg->frame->frame_offset);
                        }
                        else
                        {
                            if(bUsage == LOCAL_ALLOCATE_FOR_INPUT) sse2_movsd_xmreg_membase(inst, ejit_xmm_regs_map[count].reg, X86_EBP, vreg->frame->frame_offset);
                        }
                        ...
                        if(vreg)
                        {
                            vreg->in_frame = 0;
                            vreg->in_reg = 1;
                            vreg->reg = &ejit_xmm_regs_map[count];
                        }
                        func->ejit->scratch_regs = func->ejit->scratch_regs | ejit_xmm_regs_map[count].hash_code;
                        func->ejit->regs_state = func->ejit->regs_state | ejit_xmm_regs_map[count].hash_code;
                        bRegFound = 1;
                        if(foundReg) *foundReg = ejit_xmm_regs_map[count].reg;
                ...
                if(!bRegFound) // if we did not find any free register
                {
                    for(count = 0; count < EJIT_N_XMM_REGISTERS; count++)
                    {
                        if(ejit_xmm_regs_map[count].vreg == 0 &&
                             &ejit_xmm_regs_map[count] != reg1 && &ejit_xmm_regs_map[count] != reg2 &&
                             !(ejit_xmm_regs_map[count].hash_code & fRegCond))
                        {
                            // restore the old local register content to frame
                            ejit_vreg_t local_vreg = ejit_xmm_regs_map[count].local_vreg;
                            if(local_vreg)
                            {
                                local_vreg->in_reg = 0;
                                local_vreg->in_frame = 1;
                ...
                if(!bRegFound) // if we did not find any free register at all try to find one already saved locally
                {
                    for(count = 0; count < EJIT_N_XMM_REGISTERS; count++)
                    {
                        if(&ejit_xmm_regs_map[count] != reg1 && &ejit_xmm_regs_map[count] != reg2
                            && ejit_xmm_regs_map[count].temp_frame
                            && !(ejit_xmm_regs_map[count].hash_code & fRegCond))
                        {
                            // load new content into local register
                ...
                if(!bRegFound) // if we did not find any free register at all try to find one not saved yet locally
                {
                    for(count = 0; count < EJIT_N_XMM_REGISTERS; count++)
                    {
                        ...
                        {
                            // load new content into local register
                            ejit_frame_t frame = ejit_xmm_regs_map[count].temp_frame;
                            if(frame == 0)
                            {
                                frame = jit_memory_pool_alloc(&(func->builder->ejit_frame_pool),
                                                    struct _ejit_frame);
                                ejit_allocate_large_frame_cond6(func, frame, 8, frame1, frame2, frame3, frame4, frame5, frame6);
                                ejit_xmm_regs_map[count].temp_frame = frame;
                            }

                            sse2_movsd_membase_xmreg(inst, X86_EBP, frame->frame_offset, ejit_xmm_regs_map[count].reg);
        ...
        for(index = (num - 1); index >= 0; index--)
        {
            ...
            switch(typeKind)
            {
                ...
                default:
                {
                        ...
                        offset+=(num_frames * 4);
                        buf = (unsigned char*)ejit_memory_copy_with_reg(buf, X86_ESP, 0,
...
unsigned int ejit_stack_depth_used(jit_function_t func)
{
    int index;
    ...
    for(index = 0; index < num; index++)
    {
        jit_type_t type = ejit_type_get_param(signature, index);
        type = jit_type_remove_tags(type);
        int typeKind = jit_type_get_kind(type);
...
void ejit_compute_register_holes(jit_function_t func)
{
    ...
    jit_insn_iter_t iter;
    jit_insn_t insn;
    ejit_linked_list_t allFunctions = jit_memory_pool_alloc(&(func->builder->ejit_linked_list_pool),
                                        struct _ejit_linked_list);

    ejit_linked_list_t nativeFunctions = jit_memory_pool_alloc(&(func->builder->ejit_linked_list_pool),
                                        struct _ejit_linked_list);                
    ejit_linked_list_t holes[32];

    holes[X86_REG_EAX] = jit_memory_pool_alloc(&(func->builder->ejit_linked_list_pool),
                                        struct _ejit_linked_list);

    holes[X86_REG_EDX] = jit_memory_pool_alloc(&(func->builder->ejit_linked_list_pool),
                                        struct _ejit_linked_list);

    holes[X86_REG_ECX] = jit_memory_pool_alloc(&(func->builder->ejit_linked_list_pool),
                                        struct _ejit_linked_list);

    holes[X86_REG_EBX] = jit_memory_pool_alloc(&(func->builder->ejit_linked_list_pool),
                                        struct _ejit_linked_list);

    holes[X86_REG_ESI] = jit_memory_pool_alloc(&(func->builder->ejit_linked_list_pool),
                                        struct _ejit_linked_list);

    holes[X86_REG_EDI] = jit_memory_pool_alloc(&(func->builder->ejit_linked_list_pool),
                                        struct _ejit_linked_list);

    holes[X86_REG_XMM0] = jit_memory_pool_alloc(&(func->builder->ejit_linked_list_pool),
                                        struct _ejit_linked_list);

    holes[X86_REG_XMM1] = jit_memory_pool_alloc(&(func->builder->ejit_linked_list_pool),
                                        struct _ejit_linked_list);

    holes[X86_REG_XMM2] = jit_memory_pool_alloc(&(func->builder->ejit_linked_list_pool),
                                        struct _ejit_linked_list);
            ...
            if(insn)
            {
                switch(insn->opcode)
                {
                    ...
                    case JIT_OP_CALL_EXTERNAL_TAIL:
                    {
                        ...
                        {
                            ejit_add_item_to_linked_list(func, insn, allFunctions);
                            ejit_add_item_to_linked_list(func, insn, holes[X86_REG_EAX]);
                            ejit_add_item_to_linked_list(func, insn, holes[X86_REG_EDX]);
                            ejit_add_item_to_linked_list(func, insn, holes[X86_REG_ECX]);
                            ejit_add_item_to_linked_list(func, insn, holes[X86_REG_XMM0]);
                            ejit_add_item_to_linked_list(func, insn, holes[X86_REG_XMM1]);
                            ejit_add_item_to_linked_list(func, insn, holes[X86_REG_XMM2]);
                        }
                        else
                        {
                            ejit_add_item_to_linked_list(func, insn, allFunctions);
                            ejit_add_item_to_linked_list(func, insn, nativeFunctions);
                            ejit_add_item_to_linked_list(func, insn, holes[X86_REG_EAX]);
                            ejit_add_item_to_linked_list(func, insn, holes[X86_REG_EDX]);
                            ejit_add_item_to_linked_list(func, insn, holes[X86_REG_ECX]);
                            ejit_add_item_to_linked_list(func, insn, holes[X86_REG_XMM0]);
                    ...
                    case JIT_OP_LDIV_UN:
                    {
                            ...
                            || !jit_value_is_constant(insn->value2))
                        {
                            ...
                            ejit_add_item_to_linked_list(func, insn, holes[X86_REG_EDX]);
                            ejit_add_item_to_linked_list(func, insn, holes[X86_REG_ECX]);
                            ejit_add_item_to_linked_list(func, insn, holes[X86_REG_EBX]);
                            ejit_add_item_to_linked_list(func, insn, holes[X86_REG_ESI]);
                            ejit_add_item_to_linked_list(func, insn, holes[X86_REG_EDI]);
                        }
                    }
                    break;
                    case JIT_OP_LMUL:
                    ...
                    case JIT_OP_LOAD_RELATIVE_STRUCT:
                    {
                        ...
                        ejit_add_item_to_linked_list(func, insn, holes[X86_REG_EAX]); // At least one free register is always needed to perform a copy operation between memory.
                        ejit_add_item_to_linked_list(func, insn, holes[X86_REG_EDX]);
    ...
    func->ejit->reg_holes[X86_EAX] = holes[X86_REG_EAX];
    func->ejit->reg_holes[X86_EDX] = holes[X86_REG_EDX];
    func->ejit->reg_holes[X86_ECX] = holes[X86_REG_ECX];
    func->ejit->reg_holes[X86_EBX] = holes[X86_REG_EBX];
    func->ejit->reg_holes[X86_ESI] = holes[X86_REG_ESI];
    func->ejit->reg_holes[X86_EDI] = holes[X86_REG_EDI];

    func->ejit->reg_holes[X86_REG_XMM0] = holes[X86_REG_XMM0];
    func->ejit->reg_holes[X86_REG_XMM1] = holes[X86_REG_XMM1];
    func->ejit->reg_holes[X86_REG_XMM2] = holes[X86_REG_XMM2];
    func->ejit->reg_holes[X86_REG_XMM3] = allFunctions;
    func->ejit->reg_holes[X86_REG_XMM4] = allFunctions;
    func->ejit->reg_holes[X86_REG_XMM5] = allFunctions;
    func->ejit->reg_holes[X86_REG_XMM6] = allFunctions;
    func->ejit->reg_holes[X86_REG_XMM7] = allFunctions;
}
...
unsigned char ejit_vreg_is_in_register_hole(jit_function_t func, ejit_vreg_t vreg, unsigned int regIndex)
{
    ...
    switch(typeKind)
    {
        CASE_USE_WORD
        {
            index[0] = ejit_gp_regs_map[regIndex].reg;
        }
        break;
        CASE_USE_LONG
        {
            index[0] = ejit_gp_regs_map[regIndex].reg;
            index[1] = ejit_register_pair(index[0]);
        }
        break;
        CASE_USE_FLOAT
        {
    ...
    while(index[count] != -1)
    {
            ejit_linked_list_t list = func->ejit->reg_holes[index[count]];
...
unsigned int ejit_register_pair(unsigned int reg)
{
    return ejit_gp_regs_map[ejit_index_register_pair(gp_reg_map[reg])].reg;
}
...
ejit_reg_t ejit_object_register_pair(ejit_reg_t reg)
{
    return &(ejit_gp_regs_map[ejit_index_register_pair(gp_reg_map[reg->reg])]);
}
