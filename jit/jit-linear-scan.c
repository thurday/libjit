...
{
	lir_instance_t lir = func->lir;
	lir_vreg_t vreg = 0;
	lir_linked_list_t list;
	if(!_jit_function_ensure_builder(func) || !value)
	{
		return 0;
	}
	if(!lir_value_is_in_vreg(func, value))
	{
		if(lir->vregs_list)
		{
			list = lir->vregs_list;
			...
			list->next = jit_memory_pool_alloc(&(func->builder->lir_list_pool),
		...
		{
			lir->vregs_list = jit_memory_pool_alloc(&(func->builder->lir_list_pool),
										struct _lir_linked_list);
			list = lir->vregs_list;
		}
		vreg = jit_memory_pool_alloc(&(func->builder->lir_vreg_pool),
									struct _lir_vreg);
		vreg->value = value;
		vreg->min_range = 0;
		vreg->max_range = 0;
...
{
	lir_instance_t lir = jit_cnew(struct _lir_instance);
	lir->func = func;
	lir->cpoints_list = 0;
	lir->vregs_num = 0;
	lir->regs_state = 0;
	lir->frame_state = jit_memory_pool_alloc(&(func->builder->lir_list_pool),
								struct _lir_list);
	lir->incoming_params_state = 0;
	lir->scratch_regs = 0;
	lir->scratch_frame = 0;
	func->lir = lir;
	int index = 0;
	...
	{
		lir->reg_holes[index] = 0;
	}
	lir_init(func); // Platform dependent init.
	return lir;
}

void lir_destroy_instance(jit_function_t func)
{
	if(func && func->lir && func->lir->incoming_params_state) jit_free(func->lir->incoming_params_state);
	if(func && func->lir) jit_free(func->lir);
	func->lir = 0;
}

void lir_create_critical_point(jit_function_t func, jit_insn_t insn)
{
	lir_critical_point_t cpoint;
	if(insn && insn->cpoint==0)
	{
		cpoint = jit_memory_pool_alloc(&(func->builder->lir_critical_point_pool),
											struct _lir_critical_point);
		cpoint->is_branch_target = 0;
		cpoint->vregs_die = jit_memory_pool_alloc(&(func->builder->lir_list_pool),
											struct _lir_list);
		cpoint->vregs_born = jit_memory_pool_alloc(&(func->builder->lir_list_pool),
											struct _lir_list);
...
void lir_add_vreg_to_complex_list(jit_function_t func, lir_vreg_t vreg, lir_list_t list)
{
	if(vreg && list)
	{
		lir_linked_list_t linked_list = 0;
		...
		switch(typeKind)
		{
			CASE_USE_WORD
			{
				if(!list->item1)
				{
					list->item1 = jit_memory_pool_alloc(&(func->builder->lir_linked_list_pool),
										struct _lir_linked_list);
				}
				linked_list = (lir_linked_list_t)list->item1;
			}
			break;
			CASE_USE_LONG
			{
				if(!list->item2)
				{
					list->item2 = jit_memory_pool_alloc(&(func->builder->lir_linked_list_pool),
										struct _lir_linked_list);
				}
				linked_list = (lir_linked_list_t)list->item2;
			}
			break;
			...
			case JIT_TYPE_FLOAT64:
			{
				if(!list->item3)
				{
					list->item3 = jit_memory_pool_alloc(&(func->builder->lir_linked_list_pool),
										struct _lir_linked_list);
				}
				linked_list = (lir_linked_list_t)list->item3;
			}
			break;
			case JIT_TYPE_NFLOAT:
			{
				if(sizeof(jit_nfloat) != sizeof(jit_float64))
				{
					if(!list->item4)
					{
						list->item4 = jit_memory_pool_alloc(&(func->builder->lir_linked_list_pool),
											struct _lir_linked_list);
					}
					linked_list = (lir_linked_list_t)list->item4;
				}
				else
				{
					if(!list->item3)
		...
		if(!linked_list->next && linked_list->item != vreg)
		{
			if(linked_list->item)
			{
				linked_list->next = jit_memory_pool_alloc(&(func->builder->lir_linked_list_pool),
									    struct _lir_linked_list);
...
unsigned char lir_add_two_linked_lists(jit_function_t func, lir_linked_list_t dest_list, lir_linked_list_t source_list)
{
	unsigned char updated = 0;
	while(source_list)
	{
		...
		if(item)
		{
			...
			unsigned char new_update = lir_add_item_to_linked_list(func, item, dest_list);
...
unsigned char lir_sub_two_linked_lists(jit_function_t func, lir_linked_list_t dest_list, lir_linked_list_t source_list)
{
	unsigned char updated = 0;
	while(source_list)
	{
		...
		if(item)
		{
			unsigned char new_update = lir_remove_item_from_linked_list(func, item, dest_list);
			updated = updated || new_update;
		}
		source_list = source_list->next;
	}
...
unsigned char lir_add_add_sub_linked_lists(jit_function_t func, lir_linked_list_t dest, lir_linked_list_t list1, lir_linked_list_t list2, lir_linked_list_t list3)
{
	unsigned char updated = 0;
	// do dest = dest + a + b - c;
	lir_linked_list_t list = jit_memory_pool_alloc(&(func->builder->lir_linked_list_pool), 
								struct _lir_linked_list);
	updated = lir_add_two_linked_lists(func, list, dest);
	lir_add_two_linked_lists(func, dest, list1);
	lir_add_two_linked_lists(func, dest, list2);
	lir_sub_two_linked_lists(func, dest, list3);
	updated = lir_add_two_linked_lists(func, list, dest);
	jit_memory_pool_dealloc(&(func->builder->lir_linked_list_pool),
						    list);
	return updated;
}

unsigned char lir_remove_item_from_linked_list(jit_function_t func, void *item, lir_linked_list_t list)
{
	unsigned char updated = 0;
	if(list && item) // Cannot remove a null value.
	{
		lir_linked_list_t linked_list = list;
		if(linked_list)
		{
			while(linked_list->next && linked_list->item != item)
			{
...
unsigned char lir_update_liveness_for_branch(jit_function_t func, jit_insn_t insn_from, jit_insn_t insn_to)
{
	...
	lir_linked_list_t list = lir->vregs_list;
	lir_vreg_t vreg;
	unsigned char updated = 0;
	while(list)
	{
		vreg = (lir_vreg_t)(list->item);
		list = list->next;
		if(!jit_value_is_temporary(vreg->value) && (vreg->min_range->insn_num != vreg->max_range->insn_num))
		{
			if(vreg->min_range->insn_num <= range2 && vreg->max_range->insn_num >= range2)
			{
				if(vreg->min_range->insn_num > range1)
	    			{
        				lir_add_vreg_to_complex_list(func, vreg, insn_from->cpoint->vregs_born);
					lir_remove_vreg_from_complex_list(func, vreg, vreg->min_range->cpoint->vregs_born);
					vreg->min_range = insn_from;
					updated = 1;
				}
				else if(vreg->max_range->insn_num < range1)
				{
					lir_add_vreg_to_complex_list(func, vreg, insn_from->cpoint->vregs_die);
					lir_remove_vreg_from_complex_list(func, vreg, vreg->max_range->cpoint->vregs_die);
...
{
	lir_list_t list = func->lir->branch_list;
	lir_list_t new_list = jit_memory_pool_alloc(&(func->builder->lir_list_pool),
							struct _lir_list);
	if(list)
	{
		while(list->next)
...
{
	lir_linked_list_t linked_list;
	if(func && list && func->lir)
        {
		linked_list = (lir_linked_list_t)list->item1;
		while(linked_list)
		{
			lir_free_reg(func, linked_list->item);
			linked_list = linked_list->next;
		}
		linked_list = (lir_linked_list_t)list->item2;
		while(linked_list)
...
void lir_free_frames(jit_function_t func, lir_list_t list)
{
	...
	if(func && list && func->lir)
		...
		while(linked_list)
		{
			lir_free_vreg_frame(func, linked_list->item);
			linked_list = linked_list->next;
		}
		linked_list = (lir_linked_list_t)list->item2;
		while(linked_list)
...
lir_vreg_t lir_regs_higher_criteria(jit_function_t func, lir_vreg_t vreg1, lir_vreg_t vreg2)
{
	if(vreg1 && vreg2)
	{
...
void lir_add_branch_target(jit_function_t func, jit_insn_t insn, jit_label_t label)
{
	...
	lir_add_fixup_branch(func, insn, dest_insn);
	lir_create_critical_point(func, dest_insn);
}
...
void lir_compute_liveness(jit_function_t func)
{
	...
	if(!func->has_try)
	{
		while((block = jit_block_next(func, block)) != 0)
		{
			...
			{	
				...
				lir_create_critical_point(func, insn); // Points to which there exist branches.
			}
			jit_insn_iter_init(&iter, block);
			while((insn = jit_insn_iter_next(&iter)) != 0)
			{
				...
				{
					lir_create_critical_point(func, insn); // Point from which there is a branch.
					lir_add_branch_target(func, insn, (jit_label_t)dest);
				}
				...
				{
					if(!lir_value_is_in_vreg(func, dest))
	    				{
						lir_create_vreg(func, dest);
					}
				...
				{
					if(!lir_value_is_in_vreg(func, value1))
					{
						lir_create_vreg(func, value1);
					}
				...
				{
					if(!lir_value_is_in_vreg(func, value2))
					{
						lir_create_vreg(func, value2);
					}
		...
		linked_list = func->lir->vregs_list;
		while(linked_list)
		{
			...
			{
				lir_create_critical_point(func, vreg->min_range);
				lir_add_vreg_to_complex_list(func, vreg, vreg->min_range->cpoint->vregs_born);
				lir_create_critical_point(func, vreg->max_range);
				lir_add_vreg_to_complex_list(func, vreg, vreg->max_range->cpoint->vregs_die);
			}
		}
		...
		{
			updated = 0;
			lir_list_t last_item = func->lir->branch_list;
			list = func->lir->branch_list;
			while(list)
			{
				unsigned char new_update = lir_update_liveness_for_branch(func, (jit_insn_t)(list->item1), (jit_insn_t)(list->item2));
				updated = updated || new_update;
				list = list->next;
	...
	else // The function has a try/catch/finally block, we need to set all variables the maximum liveness range.
	{
		while((block = jit_block_next(func, block)) != 0)
		{
			...
			while((insn = jit_insn_iter_next(&iter)) != 0)
			{
				    ...
				    && !(insn->flags & JIT_INSN_DEST_IS_NATIVE))
				{
					if(!lir_value_is_in_vreg(func, dest))
	    				{
						vreg = lir_create_vreg(func, dest);					
					}
					...
					&& !value1->is_nint_constant)
				{
					if(!lir_value_is_in_vreg(func, value1))
					{
						vreg = lir_create_vreg(func, value1);
					}
					...
					&& !value2->is_constant && !value2->is_nint_constant)
				{
					if(!lir_value_is_in_vreg(func, value2))
					{
						vreg = lir_create_vreg(func, value2);
					}
		...
		lir_create_critical_point(func, min_insn);
		lir_create_critical_point(func, max_insn);
...
int lir_compile(jit_function_t func, void **entry_point)
{
	...
	func->lir = lir_create_instance(func);
	lir_compute_liveness(func);
	// lir_preallocate_global_registers(func);
	...
	for(;;)
	{
		...
		while((block = jit_block_next(func, block)) != 0)
		{
			...
			lir_compile_block(&gen, func, block);
	...
	jit_mutex_unlock(&(func->context->cache_lock));
#ifdef EJIT_DEBUG_ENABLED
	lir_debug_print_vregs_ranges(func);
#endif
