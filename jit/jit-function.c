...
int _jit_function_ensure_builder(jit_function_t func)
{
	...
	jit_memory_pool_init(&(func->builder->lir_vreg_pool), struct _lir_vreg);
	jit_memory_pool_init(&(func->builder->lir_critical_point_pool), struct _lir_critical_point);
	jit_memory_pool_init(&(func->builder->lir_list_pool), struct _lir_list);
	jit_memory_pool_init(&(func->builder->lir_frame_pool), struct _lir_frame);
	jit_memory_pool_init(&(func->builder->memory_pool), void *);
	jit_memory_pool_init(&(func->builder->lir_linked_list_pool), struct _lir_linked_list);
...
void _jit_function_free_builder(jit_function_t func)
{
	if(func->builder)
	{
		...
		jit_memory_pool_free(&(func->builder->lir_vreg_pool), 0);
		jit_memory_pool_free(&(func->builder->lir_critical_point_pool), 0);
		jit_memory_pool_free(&(func->builder->lir_list_pool), 0);
		jit_memory_pool_free(&(func->builder->lir_frame_pool), 0);
		...
		jit_memory_pool_free(&(func->builder->lir_linked_list_pool), 0);
...
compile(jit_function_t func, void **entry_point)
{
	...
	{
		return lir_compile(func, entry_point);
	}
