            ...
            for(index = (num_args - 1); index >= 0; index--)
            {
                ...
                switch(typeKind)
                {
                    CASE_USE_WORD
                    {
                        if(caller_abi == jit_abi_internal && dword_param <= 3)
                        {
                            jit_insn_outgoing_reg(func, new_args[index], ejit_regIndex2reg(func, dword_param - 1, type));
                        }
                        dword_param--;
                    }
                    break;
                    CASE_USE_FLOAT
                    {
                        if(caller_abi == jit_abi_internal && float_param <= 3)
                        {
                            jit_insn_outgoing_reg(func, new_args[index], ejit_regIndex2reg(func, float_param - 1, type));
                        }
                        float_param--;
                    }
                    break;
     ...
     jit_type_t signature, jit_value_t *args, unsigned int num_args, int flags)
{
    ...
    else
    {
        ...
        *(insn->call_params) = jit_malloc(sizeof(jit_value_t) * num_args + sizeof(struct _ejit_call_params));

        (*insn->call_params)->abi = jit_type_get_abi(signature);
        (*insn->call_params)->num = ejit_type_num_params(signature);
