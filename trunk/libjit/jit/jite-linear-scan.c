...
jite_vreg_t jite_create_vreg(jit_function_t func, jit_value_t value)
{
    ...
    if(!jite_value_is_in_vreg(func, value))
    {
        if(jite->vregs_list)
        {
...
void jite_free_frames(jit_function_t func, jite_list_t list)
{
    ...
    if(func && list && func->jite)
    {
        ...
        while(linked_list)
        {
            jite_free_vreg_frame(func, linked_list->item);
            linked_list = linked_list->next;
        }
        linked_list = (jite_linked_list_t)list->item2;
        while(linked_list)
...
jite_vreg_t jite_regs_higher_criteria(jit_function_t func, jite_vreg_t vreg1, jite_vreg_t vreg2)
{
    if(vreg1 && vreg2)
    {
...
/*
void jite_liveness_compare_nodes(jit_uint *dest_node, jit_uint *src_node1, jit_uint *src_node2, jit_int size)
{
    while( size > 0 )
    {
        size--;
